document.addEventListener('DOMContentLoaded', () => {
  ...
  if (role !== 'driver') return; // solo drivers
  ...
  const vMake  = byId('vehicle-make');
  const vModel = byId('vehicle-model');
  const vYear  = byId('vehicle-year');
  const plate  = byId('license-plate');
  ...
  vMake.value  = currentUser.make  || '';
  vModel.value = currentUser.model || '';
  vYear.value  = currentUser.year  || '';
  plate.value  = currentUser.plate || '';
